...
'''
    ...
    @staticmethod
    def CompareVersions(version1: tuple[int, int, int, int, int, int, int, int], version2: tuple[int, int, int, int, int, int, int, int]) -> int: # type: ignore
        # 比较标记
        if version1[0] != version2[0]:
            del___pycache__()
            return 1 if version1[0] == 0 else -1
        ...
        if version1[7] != version2[7]:
            ...
            if version1[7] == 0:
                del___pycache__()
                return 1 if version2[7] != 0 else 0
            elif version2[7] == 0:
                del___pycache__()
                return -1
    ...
    def JudgeVersion_Greater(Version1: str, Version2: str) -> bool:
        try:
            NV1 = VersionSystem.GetNumberVersion(Version1)
            NV2 = VersionSystem.GetNumberVersion(Version2)
        except:
            del___pycache__()
            return False
    ...
    @staticmethod
    def GetNumberVersion(version: str) -> tuple[int, int, int, int, int, int, int, int] | NoReturn:
        if VersionSystem.CheckVersion(version):
            ...
            del___pycache__()
            return (int('1' if tag == 'O' else '0'), int('0' if indev else '1'), int(major_version), int(version_number.split('.')[0]), int(version_number.split('.')[1]), int(year), int(month), int(revision))
        else:
            del___pycache__()
            raise ValueError('Invalid version format')
