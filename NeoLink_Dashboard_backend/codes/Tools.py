...
def GetContentFromGithub(name: str, rep: str, bra: str, filepath: str, use_china_mirror: bool = False) -> str:
    ...
    china_mirrors = [
        ...
        f'https://raw.kkgithub.com/'
    ]
    
    # 默认使用原始GitHub链接
    url = f'{name}/{rep}/{bra}/{filepath}'

    # 如果指定使用中国镜像，则尝试镜像站点
    if use_china_mirror:
        for mirror_url in china_mirrors:
            try:
                print(mirror_url + url)
                response = requests.get(mirror_url + url, timeout=15, verify=False)
    ...
    response = requests.get('https://raw.githubusercontent.com/' + url, timeout=15, verify=False)
    response.raise_for_status()
    return response.text
